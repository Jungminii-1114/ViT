...
def train(model, train_loader, val_loader, device, epochs=20, lr=3e-4, mixup_run=None):
    wandb.init(
    ...
    for epoch in range(epochs):
        ...
        for imgs, labels in train_loader:
            ...
            # Mixup 수행하기 
            if mixup_run is not None:
                imgs, labels = mixup_run(imgs, labels)
            # 주의 : Mix-up 거친 labels는 Soft label (확률값)으로,
            # 처리해야할 필요가 있지만, PyTorch 1.10 이상부터 CrossEntropyLoss가 처리한ㄷ고 함.
...
def main():
    ...
    '''
    * Augmentation
    train 데이터 RandomResizedCrop 말고 그냥 Resize로 해보기 

    [우려되는 점]
    transforms.Resize(224) 는 짧은 변을 224으로 맞추고, 종횡비를 유지함.
    원본 이미지의 종횡비가 다르면 최종 이미지들의 너비와 높이가 달라져버림. -> Error 발생

    - Center Crop (256 정도로 Resize 거치고, CenterCrop 하기 )
    - RandomResizedCrop 
    '''
    #transform_train = transforms.Compose([transforms.RandomResizedCrop(224), transforms.RandomHorizontalFlip(),
    #                                       transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])

    transform_train = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.RandomHorizontalFlip(), transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
    
    transform_val = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.ToTensor(),
                                        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])

    train_dataset = CustomDataset(HEAD_PATH, os.path.join(HEAD_PATH, "train.csv"), "train", transform_train)
    val_dataset = CustomDataset(HEAD_PATH, os.path.join(HEAD_PATH, "val.csv"), "val", transform_val)
    test_dataset = CustomDataset(HEAD_PATH, os.path.join(HEAD_PATH, "test.csv"), "test", transform_val)
    ...
    mixup = v2.MixUp(alpha=0.2, num_classes=30)

    # for images, labels in train_loader:
    #     images, labels = mixup(images, labels)

    train(model, train_loader, val_loader, device, lr=best_lr, mixup_run=mixup)
